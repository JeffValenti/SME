...
class Abund:
    ...
    def __call__(self, type='H=12'):
        ...
        """
        abund = OrderedDict((el,ab+self._monh if ab is not None else ab)
    ...
    def __getitem__(self, elems):
        ...
        if isinstance(elems, str):
            ...
            try:
                ...
                raise TypeError("got item descriptor '{}'".format(elem) + \
    ...
    def __setitem__(self, elem, abund):
        raise TypeError("can't set abundance directly; " + \
    ...
    def fromtype(pattern, fromtype):
        ...
        if type == 'sme':
            ...
            raise ValueError("got abundance type '{}',".format(type) +
    ...
    def set_pattern_by_name(self, pattern_name):
        if pattern_name.lower() == 'asplund2009':
            ...
            raise ValueError("got abundance pattern name '{}',". \
                   format(pattern_name) + \
    ...
    def update_pattern(self, updates):
        for key in updates:
            if key in self._pattern.keys():
                ...
                raise KeyError("got element abbreviation '{}'". \
                        format(key) + ', should be one of ' + \
    ...
    def print(self):
        if self._pattern is None:
            if self._monh is None:
                ...
                print('[M/H]={:.3f}. Abundance pattern is not set'. \
                        format(self._monh))
        else:
            if self._monh is None:
                print('[M/H] is not set. ' \
                        'Values below are the abundance pattern.')
                a = self.get_pattern('H=12')
            else:
                print('[M/H]={:.3f} applied to abundance pattern. ' \
                        'Values below are abundances.'. \
                        format(self._monh))
                a = self.get_pattern('H=12')
                a = a[0:2] + [ab+self._monh for ab in a[2:]]
